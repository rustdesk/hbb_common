...
// FramedStream::new and FramedSocket::new take caller-supplied local addresses;
// those must never be pinned to the configured device, or a connection that
// used to work (e.g. over loopback) gets filtered onto the wrong link.
#[tokio::test]
async fn caller_supplied_local_address_is_not_pinned() {
    let _guard = lock();
    let _restore = BindRestore::save();
    ...
    // an interface that exists but is not the loopback path to the listener
    let device = hbb_common::config::Config::get_option(keys::OPTION_BIND_INTERFACE);
    drop(device);
    set_bind("lo");

    let local: std::net::SocketAddr = "127.0.0.1:0".parse().unwrap();
    let stream = socket_client::connect_tcp_local(format!("127.0.0.1:{port}"), Some(local), 3000)
        .await
        .expect("a caller-supplied local address must still connect");
    assert_eq!(stream.local_addr().ip().to_string(), "127.0.0.1");
}
